...
def load_data(image_file, label_file):
    ...
    label= label_data['map']
    # w, h = gt1.shape
    # label = np.zeros_like(gt1)
    # label[(gt1 == 2)[:w, :h]] = 1
    # label[(gt1 == 5)[:w, :h]] = 2
    # label[(gt1 == 7)[:w, :h]] = 3
    # label[(gt1 == 4)[:w, :h]] = 4
...
def getDataAndLabels(trainfn1, trainfn2):
    ...
    label_dioni=imdb['Labels']
    print(np.max(imdb['Labels']),np.count_nonzero(label_dioni==3))
    imdb['set'] = np.ones([nSample]).astype(np.int64)
    print('Data is OK.')

    return imdb

# train_data_file = './datasets/PaviaU/PaviaU.mat'
# train_label_file = './datasets/PaviaU/PaviaU_gt.mat'
# train_data_file = './datasets/HangZhou/2.mat'
# train_label_file = './datasets/HangZhou/2_gt.mat'
train_data_file = './datasets/Dioni/Dioni.tif'
train_label_file = './datasets/Dioni/Dioni_gt_out68.mat'
...
with open('datasets/Dioni_imdb_128.pickle', 'wb') as handle:
